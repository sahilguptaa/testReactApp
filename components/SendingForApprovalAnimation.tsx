...
const SENDING_STEPS = [
  'Creating secure collaboration channel...',
  'Adding supplier to the channel...',
  'Adding Sourcing Manager to channel...',
  'Sending award notification...',
];

interface SendingForApprovalAnimationProps {
    onComplete?: () => void;
    onStepChange?: (stepText: string) => void;
}

export const SendingForApprovalAnimation: React.FC<SendingForApprovalAnimationProps> = ({ onComplete, onStepChange }) => {
  ...
  useEffect(() => {
    ...
    if (onStepChange && currentStep < SENDING_STEPS.length) {
        onStepChange(SENDING_STEPS[currentStep]);
    }
  ...
  return (
    <div>
      <p className="font-medium mb-3">Sending award for approval...</p>
