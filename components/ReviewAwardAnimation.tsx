...
const GATHERING_STEPS = [
  'Fetching data from Sourcing Decision Engine (SDE)...',
  'Fetching data from Enterprise Quote (EQ)...',
  'Validating Vendor ID...',
  'Fetching supplier profile...',
  'Finalizing award...',
  'Creating award preview...',
];

export const ReviewAwardAnimation: React.FC<{ onComplete?: () => void }> = ({ onComplete }) => {
  const [currentStep, setCurrentStep] = useState(0);

  useEffect(() => {
    const totalDuration = 5000;
    const stepDuration = totalDuration / GATHERING_STEPS.length;
    
    if (currentStep >= GATHERING_STEPS.length) {
      if (onComplete) {
  ...
  return (
    <div>
      <p className="font-medium mb-3">Gathering award information...</p>
      <div className="space-y-2">
        {GATHERING_STEPS.map((step, index) => (
